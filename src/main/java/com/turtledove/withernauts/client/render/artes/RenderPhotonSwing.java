...
public class RenderPhotonSwing implements ArteRenderBase {
    ...
    public void renderSwordInFirstPerson(AbstractClientPlayer player, float partialTicks, float p_187457_3_, EnumHand hand, float swing_ratio, ItemStack stack, float equip_ratio)
    {
        ...
        if (data.get_current_stage() == 0)
            GlStateManager.translate(-0.1F,  -0.40f, -0.5f);
        else
            GlStateManager.translate(-0.1F,  -0.40f + swing_ratio * swing_ratio * 0.5f, -0.5f);
    ...
    public void transformSwordSideFirstPerson(EnumHandSide hand, float equip_ratio,  float swing_ratio)
    {
        ...
        if (data.get_current_stage() == 0)
        {
            float f1 = MathHelper.cos((1.0f - swing_ratio) * (1.0f - swing_ratio) *(float)Math.PI);
            float f2 = MathHelper.sin((swing_ratio) * (swing_ratio) * (float)Math.PI);
            GlStateManager.rotate( 90.0F*f1, 0.0F, 1.0F, 0.0F);
            GlStateManager.rotate(45.0F*f1, 1.0F, 0.0F, 0.0F);
        }
        else
        {
