...
public class RenderSwordRainAlpha implements ArteRenderBase
{
    ...
    public void transformSwordSideFirstPerson(EnumHandSide hand, float equip_ratio,  float swing_ratio)
    {
        ...
        PlayerData data = (PlayerData) NecropolisPlayerData.get(Minecraft.getMinecraft().player);
        float xOffset = (data.get_current_stage() % 4.0F) * (float)Math.pow(-1,data.get_current_stage())/ 6.0F;

        if (data.get_current_stage() == 6 )
        {
            GlStateManager.translate((float)i * 0.125F  - swing_ratio * 0.75F + 0.1F, swing_ratio - 1.2F, -0.72F);
        ...
        {
            GlStateManager.translate(0.0F + xOffset,  -0.25f, -f - 1F);
