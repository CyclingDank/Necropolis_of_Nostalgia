package com.turtledove.withernauts.client.render.artes;
...
public class RenderBeastSwing implements ArteRenderBase
{
    public void renderArte(AbstractClientPlayer abstractclientplayer, ItemStack item, float partialTicks)
    {
        ...
        if (flag)
        {
            ...
            this.renderSwordInFirstPerson(abstractclientplayer, partialTicks, f1, EnumHand.MAIN_HAND, swing_ratio, item, equip_ratio);
        }
    ...
    public void renderSwordInFirstPerson(AbstractClientPlayer player, float partialTicks, float p_187457_3_, EnumHand hand, float swing_ratio, ItemStack stack, float equip_ratio)
    {
        ...
        if (data.get_current_stage() == 0)
        {
            int j = 1;
            float f = -0.8F * MathHelper.sin(MathHelper.sqrt(swing_ratio) * (float) Math.PI * 2F);
            ...
            int i = 1;
            GlStateManager.translate(0.0F,  -0.25f, -f - 0.5F);
        }
        else
        {
            int j = 1;
            ...
            float f2 = -0.2F * MathHelper.sin(swing_ratio * (float) Math.PI);
            int i = 1;
            GlStateManager.translate((float) i * f*0.8F + 0.7F,  swing_ratio * 0.5, f1 + 0.2F);
        }
    ...
    public void transformSwordSideFirstPerson(EnumHandSide hand, float equip_ratio,  float swing_ratio)
    {
        ...
        //GlStateManager.translate((float)i * -0.52F  + swing_ratio* 0.8F, -0.52F + swing_ratio * 0.1F, -0.22F + swing_ratio * -0.5);
        PlayerData data = (PlayerData) NecropolisPlayerData.get(Minecraft.getMinecraft().player);
        if (data.get_current_stage() == 0)
        {
            ...
            GlStateManager.translate((float)i * -0.22F  + swing_ratio * 1.25F, -0.42F + swing_ratio * -0.3F, -0.72F);
        //GlStateManager.translate((float)i * 0.52F  - swing_ratio* 0.8F, -0.22F + swing_ratio * -0.3F, -0.72F);
    }
    public void transformSwordFirstPerson(EnumHandSide hand, float swing_ratio)
    {
        PlayerData data = (PlayerData) NecropolisPlayerData.get(Minecraft.getMinecraft().player);
        if (data.get_current_stage() == 1)
        {
            int i = hand == EnumHandSide.RIGHT ? 1 : -1;
            float swing_progress = 1.0f - swing_ratio;
            float f = MathHelper.cos(swing_progress * swing_progress * (float)Math.PI);
            GlStateManager.rotate(-90.0F, 0.0F, 1.0F, 0.0F);
            GlStateManager.rotate(90.0F, 0.0F, 0.0F, 1.0F);

            GlStateManager.rotate(f * -80.0F, 1.0F, 0.0F, 0.0F);
            float f1 = MathHelper.sin(MathHelper.sqrt(swing_progress) * (float)Math.PI);
            GlStateManager.rotate((float)i * f1 * -20.0F, 0.0F, 0.0F, 1.0F);
            GlStateManager.rotate((float)i * (45.0F + f * -15.0F), 0.0F, 1.0F, 0.0F);
