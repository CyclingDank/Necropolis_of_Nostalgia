...
public class RenderHurricaneBeast  implements ArteRenderBase
{
    ...
    public void transformSwordSideFirstPerson(EnumHandSide hand, float equip_ratio,  float swing_ratio)
    {
        ...
        PlayerData data = (PlayerData) NecropolisPlayerData.get(Minecraft.getMinecraft().player);
        if (data.get_current_stage() == 0 || data.get_current_stage() == 2)
        {
            GlStateManager.translate((float)i * 0.125F  - swing_ratio * 0.75F + 0.3F, swing_ratio - 0.7F, -0.52F);
        ...
        {
            int j = 1;
            f = -0.8F * MathHelper.sin(MathHelper.sqrt(swing_ratio) * (float) Math.PI);
            ...
            f = MathHelper.cos(swing_progress * swing_progress * (float)Math.PI);
            GlStateManager.rotate(-90.0F, 0.0F, 1.0F, 0.0F);
            GlStateManager.rotate(90.0F, 0.0F, 0.0F, 1.0F);

            GlStateManager.rotate(f * -80.0F, 1.0F, 0.0F, 0.0F);
            f1 = MathHelper.sin(MathHelper.sqrt(swing_progress) * (float)Math.PI);
