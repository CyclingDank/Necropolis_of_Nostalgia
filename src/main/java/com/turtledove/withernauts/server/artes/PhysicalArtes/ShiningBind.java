...
public class ShiningBind extends PhysicalArte
{
    public ShiningBind()
    {
        super(new Vec3d(0.0D,0.0D,0.0D),new int[]{4},4,new Vec3d(0.0D,0.0D,0.0D),20, 0);
        this.arte_name = "Shining Bind";
        this.arte_desc = "Base Arte: Casts a light of judgement to those struck by ones blade.";
    }
    @Override
        ...
        if (!swipeEnemies.isEmpty())
        {
            ...
            for (Entity entity : swipeEnemies)
            {
                if (entityInSlash(player, entity) == true)
                {
                    ...
                    Vec3d landPos = swipeEnemies.get(0).getPositionVector();
                    EntityJudgement entityJudgement1 = new EntityJudgement(this.player.world, this.player, landPos, this.getAttackDamage(false, 3.0F));
                    entityJudgement1.setPosition(landPos.x, landPos.y + 20.0D, landPos.z);
                    this.player.world.spawnEntity(entityJudgement1);
                }
            }
        }
