...
public class EntityJudgement extends EntityCasted
{
    private int ticksAlive;
    private float damage;
    private int deathCounter;

    public EntityJudgement(World worldIn)
    {
        super(worldIn);
        this.setSize(3.0F, 0.25F);
    }
    public EntityJudgement (World worldIn, EntityPlayer player, Vec3d pos, float damage)
    {
    ...
    public void onUpdate()
    {
        ...
        if (this.posY <= this.caster.posY)
        {
            if (Minecraft.getMinecraft().world.getBlockState(this.getPosition()).getBlock() != Blocks.AIR)
            {
                ...
                {
                    this.playSound(NecropolisSounds.LIGHT_CAST, 0.5F,1.0F);
                    int validPos = 0;
                    for (int i = 0; i < 255; i++)
                    {
                    ...
                    EntityGroundStrike entityGroundStrike = new EntityGroundStrike(this.world, (EntityPlayer)this.caster, strikePos,7, this.damage);
