...
public class AnimationBGPunchSwipeAI<T extends NecropolisEntity & IAnimatedEntity> extends AnimationAI<T> {
    public AnimationBGPunchSwipeAI(T entity, Animation animation) {
    ...
    public void updateTask() {
        ...
        {
            this.entity.playSound(SoundEvents.ENTITY_ENDERMEN_HURT, 0.7F, 1.4F);
        }
        if (entity.getAnimationTick() == 6 || entity.getAnimationTick() == 12)
        {
            this.entity.playSound(SoundEvents.ENTITY_PLAYER_ATTACK_SWEEP, 0.7F, 0.7F);
        }
        if (entity.getAnimationTick() == 6 || entity.getAnimationTick() == 8 || entity.getAnimationTick() == 14)
        {
                ...
                if (targ instanceof EntityPlayer) {
                    if (entityInSlash(entity, targ) == true) {
                        ...
                        if (targ instanceof EntityPlayer)
                        {
                            ...
                            {
                                Necropolis_of_Nostalgia.packetHandler.sendTo(new SyncPlayer(0.0D, 0.5F, 0.0D),(EntityPlayerMP)targ);
                                targ.motionY = 0.5F;
                            }
                            else
                            {
