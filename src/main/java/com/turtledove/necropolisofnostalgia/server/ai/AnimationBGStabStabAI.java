...
public class AnimationBGStabStabAI<T extends NecropolisEntity & IAnimatedEntity> extends AnimationAI<T> {
    public AnimationBGStabStabAI(T entity, Animation animation) {
    ...
    public void updateTask() {
        ...
        if (entity.getAnimationTick() == 6 || entity.getAnimationTick() == 26)
        {
            if (entity.getAnimationTick() == 6)
                this.entity.playSound(SoundEvents.ENTITY_ENDERMITE_DEATH, 1.0F, 1.0F);
    ...
    public boolean entityInSlash(Entity source, Entity target) {
        ...
        Vec3d P = new Vec3d(-1.0, -0.3, 3.0);
        Vec3d R = new Vec3d(0.5, -0.3, 2.5);

        Q = new Vec3d(-1.0, -0.3, 0.0);
        P = new Vec3d(-1.0, -0.3, 3.0);
        R = new Vec3d(1.0, -0.3, 3.0);
