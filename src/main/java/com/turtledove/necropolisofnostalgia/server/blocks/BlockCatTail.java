...
import com.turtledove.necropolisofnostalgia.server.item.ItemHandler;
...
public class BlockCatTail extends BlockBush
{
    ...
    public BlockCatTail()
    {
        ...
        this.setRegistryName("cat_tail");
        //this.setCreativeTab(CreativeTabs.DECORATIONS);
        this.setUnlocalizedName("cat_tail");
    ...
    public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state)
    {
        if (pos.getY() >= 0 && pos.getY() < 256)
        {
            ...
            return worldIn.getBlockState(pos.down()).getMaterial() == Material.WATER && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0
                    && (worldIn.getBlockState(pos.down().down()).getMaterial() == Material.GROUND || worldIn.getBlockState(pos.down().down()).getMaterial() == Material.SAND
            || worldIn.getBlockState(pos.down().down()).getMaterial() == Material.CLAY);
        }
        else
        {
            return false;
