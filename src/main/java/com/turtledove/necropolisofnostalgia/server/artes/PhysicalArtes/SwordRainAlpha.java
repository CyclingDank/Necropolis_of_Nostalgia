...
public class SwordRainAlpha extends PhysicalArte {
    public SwordRainAlpha() {
        super(new Vec3d(0.0D, 0.0D, 0.0D), new int[]{3, 3, 3, 3, 3, 3, 6}, 24, new Vec3d(0.0D, 0.0D, 0.0D), 20, 0);
        this.setArteWeapon(1);
        this.arte_name = "Sword Rain Alpha";
        this.arte_desc = "Altered Arte: A lightning fast lunge that sends foes into the air.";

        this.isBase = false;
        this.tUsage = new ArteUsage[1];
        this.tUsage[0] = new ArteUsage(15, 100);
    }
    @Override
        ...
        if (!swipeEnemies.isEmpty())
        {
            for (Entity entity : swipeEnemies)
            {
                if (entityInSlash(player, entity) == true)
                {
                    ...
                    if (arte_stage == 6)
                        entity.addVelocity(0.0D, 0.8D,0.0D);
                }
            }
        }
