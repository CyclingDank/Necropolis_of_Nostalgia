...
public class AnimationFuguLeapFlailAI<T extends NecropolisEntity & IAnimatedEntity> extends AnimationAI<T> {
    public AnimationFuguLeapFlailAI(T entity, Animation animation)
    {
    ...
    public void updateTask()
    {
        ...
        if (this.entity.getAnimationTick() == 30 || this.entity.getAnimationTick() == 32)
        {
            this.entity.playSound(SoundEvents.ENTITY_PLAYER_ATTACK_SWEEP, 0.7F, 0.7F);
        }
        if (this.entity.getAnimationTick() == 32 || this.entity.getAnimationTick() == 34)
        {
            List<Entity> swipeEnemies = this.entity.world.getEntitiesWithinAABBExcludingEntity(entity,  entity.getEntityBoundingBox().expand(6.0,1.0,6.0).offset(-3D,0D,-3D));
            hitTargets(swipeEnemies);
        }
    }
    @Override
        ...
        if (!swipeEnemies.isEmpty())
        {
            for (Entity targ : swipeEnemies)
            {
                if (targ instanceof EntityPlayer)
                {
                    if (this.entity.getAnimationTick() == 32 || this.entity.getAnimationTick() == 34)
                    {
                        if (entityInSlash(this.entity, targ))
                            targ.attackEntityFrom(DamageSource.causeMobDamage(this.entity),(float) Necropolis_of_Nostalgia.CONFIG.fuguDamage);
                    }
                    else
                        targ.attackEntityFrom(DamageSource.causeMobDamage(this.entity),(float) Necropolis_of_Nostalgia.CONFIG.fuguDamage);
                }
    ...
    public boolean entityInSlash(Entity source, Entity target) {
        ...
        Q = new Vec3d(-1.5, -0.3, -2.5);
        P = new Vec3d(-1.5, -0.3, 0.0);
        R = new Vec3d(1.5, -0.3, 0.0);
