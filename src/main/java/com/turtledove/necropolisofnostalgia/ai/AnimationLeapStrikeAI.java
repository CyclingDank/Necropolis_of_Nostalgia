...
public class AnimationLeapStrikeAI <T extends NecropolisEntity & IAnimatedEntity> extends AnimationAI<T> {
    public AnimationLeapStrikeAI(T entity, Animation animation)
    {
    ...
    public void updateTask()
    {
        ...
        if (this.entity.getAnimationTick() == 0)
        {
            ...
            if (this.entity.targetofmyrevenge.posY - this.entity.posY == 0)
                this.entity.motionY = 0.25F;
            else
                this.entity.motionY = Math.abs(0.25F * Math.min((this.entity.targetofmyrevenge.posY - this.entity.posY), 2));
            this.entity.motionX = slopeX * 0.75F;
            this.entity.motionZ = slopeZ * 0.75F;
            this.entity.playSound(NecropolisSounds.SKELETON_ROAR,1.0f,1.0f);
        }
        if (this.entity.getAnimationTick() == 12)   //first strike
        {
            entity.playSound(SoundEvents.ENTITY_PLAYER_ATTACK_SWEEP,1.0F, 0.5F);
            List<Entity> swipeEnemies = this.entity.world.getEntitiesWithinAABBExcludingEntity(entity,  entity.getEntityBoundingBox().expand(5.0,1.0,5.0).offset(-3.5D,0D,-3.5D));
            hitTargets(swipeEnemies);
        }
        /*else if (this.entity.getAnimationTick() == 22)  //first strike
        {
        ...
        if (!swipeEnemies.isEmpty())
        {
            for (Entity targ : swipeEnemies)
            {
                if (targ instanceof EntityPlayer)
                {
                    ...
                    {
                        boolean hitTest = targ.attackEntityFrom(DamageSource.causeMobDamage(this.entity),20.0F);
                    }
                }
            }
        }
