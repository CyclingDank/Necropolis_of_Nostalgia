...
public class AnimationSpiderRearJab <T extends NecropolisEntity & IAnimatedEntity> extends AnimationAI<T> {
    public AnimationSpiderRearJab(T entity, Animation animation)
    {
    ...
    public void updateTask() {
        ...
        if (this.entity.getAnimationTick() == 4 || this.entity.getAnimationTick() == 11 || this.entity.getAnimationTick() == 18)
        {
            entity.playSound(SoundEvents.ENTITY_PLAYER_ATTACK_SWEEP,1.0F, 1.5F);
            List<Entity> swipeEnemies = this.entity.world.getEntitiesWithinAABBExcludingEntity(entity,  entity.getEntityBoundingBox().expand(5.0,1.0,5.0).offset(-2D,0D,-2D));
        ...
        if (!swipeEnemies.isEmpty())
        {
            for (Entity targ : swipeEnemies)
            {
                if (targ instanceof EntityPlayer)
                {
                    ...
                    {
                        boolean hitTest = targ.attackEntityFrom(DamageSource.causeMobDamage(this.entity),(float) Necropolis_of_Nostalgia.CONFIG.spiderDamage);
                        ((EntityPlayer) targ).addPotionEffect(new PotionEffect(MobEffects.POISON,160));
                    }
                }
            }
        }
    ...
    public boolean entityInSlash(Entity source, Entity target, int stage)
    {
        ...
        Vec3d R = new Vec3d(0.5,-0.3,1.5);

        if (stage == 4)
        {
            Q = new Vec3d(0.5,-0.3,0.0);
            P = new Vec3d(0.5,-0.3,1.5);
            R = new Vec3d(1.5,-0.3,1.5);
        }
        else if (stage == 11)
        {
            Q = new Vec3d(-0.5,-0.3,0.0);
            P = new Vec3d(-0.5,-0.3,1.5);
            R = new Vec3d(0.50,-0.3,1.5);
        }
        else if (stage == 18)
        {
            Q = new Vec3d(-1.5,-0.3,0.0);
            P = new Vec3d(-1.5,-0.3,1.5);
