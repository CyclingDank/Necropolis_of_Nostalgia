...
public class AnimationBooglinJabAI   <T extends NecropolisEntity & IAnimatedEntity> extends AnimationAI<T> {
    public AnimationBooglinJabAI(T entity, Animation animation) {
    ...
    public void updateTask()
    {
        ...
        else if (entity.getAnimationTick() == 5)
        {
            this.entity.playSound(NecropolisSounds.BOOGLIN_DRILL_2, 0.5F, 1.0F);
        }
    }
            ...
            for (Entity targ : swipeEnemies) {
                if (targ instanceof EntityPlayer)
                {
                    if (entityInSlash(entity, targ) == true) {
                        boolean hitTest = targ.attackEntityFrom(DamageSource.causeMobDamage(this.entity), (float) Necropolis_of_Nostalgia.CONFIG.booglinDamage);
                    }
                }
            }
        }
