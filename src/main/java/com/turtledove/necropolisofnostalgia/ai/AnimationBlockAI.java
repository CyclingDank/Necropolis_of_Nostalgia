    ...
    public void updateTask()
    {
            ...
            if (!swipeEnemies.isEmpty())
            {
                for (Entity targ : swipeEnemies)
                {
                    if (targ instanceof EntityPlayer)
                    {
                        ...
                        {
                            boolean hitTest = targ.attackEntityFrom(DamageSource.causeMobDamage(this.entity),(float) Necropolis_of_Nostalgia.CONFIG.skeletonDamage);
                        }
                    }
                }
            }
        ...
        else if (this.entity.getAnimationTick() == 37 || this.entity.getAnimationTick() == 38)
        {
            ...
            if (!swipeEnemies.isEmpty())
            {
                ...
                {
                    if (targ instanceof EntityPlayer)
                        targ.attackEntityFrom(DamageSource.causeMobDamage(this.entity),(float) Necropolis_of_Nostalgia.CONFIG.skeletonDamage);
                }
            }
        }
    }
